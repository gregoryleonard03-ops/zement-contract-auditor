...
st.set_page_config(
    page_title="Zement Contract Auditor",
    page_icon="🔍",
    layout="wide",
)
...
SAMPLE_DIR = Path(__file__).parent / "tests" / "sample_contracts"
...
def render_header():
    ...
    st.markdown(
        ...
        <div class="header-banner">
            ...
            <p>AI-powered risk analysis for vendor contracts — upload a PDF or DOCX to get started</p>
        </div>
        """,
...
def render_summary_stats(results: list):
    ...
    c2.metric("High Risk", high_risk, delta=None)
    ...
    with st.sidebar:
        ...
        if st.button("Load Demo Contracts", use_container_width=True):
            ...
            if samples:
                ...
                st.warning("Sample contracts not found. Run `python tests/generate_test_contracts.py` first.")
